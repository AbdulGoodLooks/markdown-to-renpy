...
def defined_character_script(line:str, character_tag:str, character_name:str) -> str:
    """
    Returns a string in the format
    `character "text"`
    """
    split_line = line.lstrip(character_name)
    split_line = split_line.lstrip(" :").split()
    # Process speaker.
    ...
    for word in split_line:
        joint_line = joint_line + " " + word
    text = text_script(joint_line.strip())
...
for line in characters_file:
    if line.strip().startswith('define'):
        if 'Character("' in line:
            ...
            character_variable = line.split('=')[0].strip()
            # grab the full name
            character_full_name = line.split('"')[1].strip()
...
# Custom configuration will parse the text and make edits before anything else.
allow_custom_edits = True
if allow_custom_edits:
    print("Custom edits applied.")
    # replace_strings = {"***": "\n",}
    # comment_symbol = "*" # Anything wrapped in this symbol will be treated as a comment.
...
with open(output_filename, "w", encoding = "utf-8-sig") as output_file:
    ...
    for line in file:
        ...
        # Parse the text and make custom edits.
        if allow_custom_edits:
            if line == "***":
                line_processed = True
            line = line.replace("***", "\nscene [insert_scene]\n")
        ...
        if line_processed:
            ...
            # For choices, anything matching the pattern `**number` is a choice 
                ...
                for tag, character in all_characters.items():
                    if line.startswith(character):
                        line = defined_character_script(line, tag, character)
                        line_processed = True
